...
const Home = () => {
	const { setCursorSettings, preloader } =
		useContextProvider();
	...
	useEffect(() => {
		const headerTextSplit = SplitType.create(
			headerTextRef.current
		);
		const infoSplit = SplitType.create(
			infoRef.current
		);
		const infoSplit2 = SplitType.create(
			infoRef2.current
		);
	...
	return (
		<RouteTransition>
			<Container>
				<Hero ref={heroRef}>
					<ContentWrapper>
						<HeaderText ref={headerTextRef}>
							Clever Studio
						</HeaderText>
						<Content>
							<div className='info'>
								<h2 ref={infoRef}>
									Clever Studio, your one-stop destination
									for cutting-edge creative media
									solutions.
								</h2>
								<p ref={infoRef2}>
									Our team of talented designers,
									developers, and branding experts work
									tirelessly to bring your vision to life.
									Whether you're looking to create a
									stunning website that captures your brand
									essence.
								</p>
								<span onClick={() => navigate('/quote')}>
									Reach us
									<FaArrowRight />
								</span>
							</div>
